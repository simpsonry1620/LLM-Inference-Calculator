...
def test_estimate_completion_latency():
    ...
    expected_token_latency = (flops_per_token / (gpu_tflops * 1e12)) / efficiency
    expected_generation_latency = expected_token_latency * output_length
    expected_total_latency = expected_prefill_latency + expected_generation_latency
    expected_tokens_per_second = 1.0 / expected_token_latency
...
def test_history_callback():
    ...
    assert "Completion_Latency" in history[2]
...
def test_edge_cases():
    ...
    small_result = calculator.estimate_prefill_latency(
        flops_prefill=1,
        gpu_tflops=1000
    )
