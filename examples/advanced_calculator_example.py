...
def analyze_model(
    ...
    # Estimate parameters
    attention_params = (
        4 * hidden_dim * hidden_dim * num_layers
    ...
    layernorm_params = (
        4 * hidden_dim * num_layers + 2 * hidden_dim
    )  # Layer norms + final LN
    total_params = attention_params + ff_params + embedding_params + layernorm_params

    print_model_stats(
        name, total_params, hidden_dim, ff_dim, num_layers, seq_len, batch_size
    )
