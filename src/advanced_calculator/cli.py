...
@click.option(
    "--precision",
    type=click.Choice(["fp16", "fp32", "bf16"]),
    default="fp16",
    help="Model precision",
)
@click.option(
    "--tflops", type=float, default=312.0, help="GPU TFLOPS (e.g., 312 for A100)"
)
...
def calculate(
    ...
    # FLOPs calculations
    flops_attention = calculator.calculate_flops_attention(
        batch_size, seq_len, hidden_dim
    )
    flops_feedforward = calculator.calculate_flops_feedforward(
        batch_size, seq_len, hidden_dim, ff_dim
    )
    flops_prefill = calculator.calculate_flops_prefill(
        batch_size, seq_len, hidden_dim, ff_dim, num_layers
    )
    ...
    tokens_per_second = calculator.estimate_inference_throughput(
        flops_per_token, tflops, efficiency
    )
    prefill_latency = calculator.estimate_prefill_latency(
        flops_prefill, tflops, efficiency
    )
    ...
    if format == "json":
        ...
        tables.append("\nModel Parameters:")
        tables.append(
            tabulate(model_params, headers=["Parameter", "Value"], tablefmt="grid")
        )
        ...
        tables.append(
            tabulate(compute_reqs, headers=["Metric", "Value"], tablefmt="grid")
        )
        ...
        tables.append("\nMemory Requirements:")
        tables.append(
            tabulate(memory_reqs, headers=["Component", "VRAM"], tablefmt="grid")
        )
...
@click.option(
    "--seq-len",
    type=int,
    help="Input sequence length (uses model default if not specified)",
)
@click.option(
    "--output-len", type=int, default=128, help="Desired output sequence length"
)
@click.option("--batch-size", type=int, default=1, help="Batch size")
@click.option(
    "--precision",
...
def analyze_model(
    model_name, seq_len, output_len, batch_size, precision, tflops, efficiency, format
):
    ...
    try:
        ...
        if format == "json":
            ...
            results["flops"]["prefill_total"] = (
                f"{results['flops']['prefill_total']:.2e}"
            )
            ...
            results["performance"]["tokens_per_second"] = (
                f"{results['performance']['tokens_per_second']:.2f}"
            )
            results["performance"]["prefill_latency"] = (
                f"{results['performance']['prefill_latency']:.4f} s"
            )
            results["performance"]["token_latency"] = (
                f"{results['performance']['token_latency']:.4f} s"
            )
            results["performance"]["time_to_first_token"] = (
                f"{results['performance']['time_to_first_token']:.4f} s"
            )
            results["performance"]["total_request_time"] = (
                f"{results['performance']['total_request_time']:.2f} s"
            )
            results["performance"]["time_for_1000_tokens"] = (
                f"{results['performance']['time_for_1000_tokens']:.2f} s"
            )

            # Fix throughput values for JSON display
            for gpu, value in results["performance"]["throughput_by_gpu"].items():
                results["performance"]["throughput_by_gpu"][gpu] = (
                    f"{value:.2f} tokens/s"
                )
            ...
            tables.append("\nModel Information:")
            tables.append(
                tabulate(model_info, headers=["Attribute", "Value"], tablefmt="grid")
            )

            # Analysis Parameters Table
            analysis_params = [
                [k, v] for k, v in results["analysis_parameters"].items()
            ]
            tables.append("\nAnalysis Parameters:")
            tables.append(
                tabulate(
                    analysis_params, headers=["Parameter", "Value"], tablefmt="grid"
                )
            )
            ...
            tables.append("\nFLOPs Requirements:")
            tables.append(
                tabulate(flops_table, headers=["Component", "FLOPs"], tablefmt="grid")
            )
            ...
            tables.append("\nVRAM Requirements:")
            tables.append(
                tabulate(vram_table, headers=["Component", "VRAM"], tablefmt="grid")
            )
            ...
            tables.append("\nPerformance Estimates:")
            tables.append(
                tabulate(perf_table, headers=["Metric", "Value"], tablefmt="grid")
            )

            # GPU Comparison Table
            gpu_comparison = [
                [gpu, f"{tps:.2f} tokens/s"]
                for gpu, tps in results["performance"]["throughput_by_gpu"].items()
            ]
            tables.append("\nPerformance Across GPUs:")
            tables.append(
                tabulate(gpu_comparison, headers=["GPU", "Tokens/s"], tablefmt="grid")
            )
...
@click.option(
    "--precision",
    type=click.Choice(["fp16", "fp32", "bf16", "int8", "int4"]),
    default="fp16",
    help="Model precision",
)
@click.option("--efficiency", type=float, default=0.3, help="GPU efficiency factor")
...
def analyze_model_on_gpu(
    model_name, gpu_name, seq_len, output_len, batch_size, precision, efficiency, format
):
    ...
    try:
        ...
        if format == "json":
            ...
            gpu_info = [
                ...
                [
                    f"{precision.upper()} TFLOPs",
                    results["gpu_info"].get(f"{precision}_tflops", "N/A"),
                ],
                [
                    "Memory Bandwidth",
                    f"{results['gpu_info']['bandwidth_gb_per_sec']} GB/s",
                ],
                ["TDP", f"{results['gpu_info'].get('tdp_watts', 'N/A')} W"],
                [
                    "Tensor Cores",
                    "Yes"
                    if results["gpu_info"].get("has_tensor_cores", False)
                    else "No",
                ],
            ...
            tables.append("\nGPU Information:")
            tables.append(
                tabulate(gpu_info, headers=["Attribute", "Value"], tablefmt="grid")
            )
            ...
            analysis_params = [
                ...
                [
                    "Precision Supported",
                    "Yes"
                    if results["analysis_parameters"]["precision_supported"]
                    else "No",
                ],
                [
                    "Efficiency Factor",
                    results["analysis_parameters"]["efficiency_factor"],
                ],
            ]
            tables.append("\nAnalysis Parameters:")
            tables.append(
            ...
            compatibility = [
                [
                    "VRAM Required",
                    f"{results['compatibility']['vram_required']:.2f} GB",
                ],
                [
                    "VRAM Available",
                    f"{results['compatibility']['vram_available']:.2f} GB",
                ],
                [
                    "Model Fits",
                    "Yes" if results["compatibility"]["vram_fits"] else "No",
                ],
                [
                    "VRAM Headroom",
                    f"{results['compatibility']['vram_headroom_gb']:.2f} GB",
                ],
                ["Maximum Batch Size", results["compatibility"]["maximum_batch_size"]],
                [
                    "Maximum Sequence Length",
                    results["compatibility"]["maximum_sequence_length"],
                ],
            ]
            tables.append("\nCompatibility Assessment:")
            tables.append(
                tabulate(compatibility, headers=["Metric", "Value"], tablefmt="grid")
            )
...
@click.option(
    "--min-headroom-gb", type=float, default=2.0, help="Minimum VRAM headroom in GB"
)
@click.option(
    "--format",
    type=click.Choice(["table", "json"]),
...
def recommend_gpus(model_name, vram_gb, min_headroom_gb, format):
    ...
    if not model_name and vram_gb is None:
        click.echo(
            "Error: Either --model-name or --vram-gb must be specified", err=True
        )
        return
    ...
    try:
        ...
        recommended_gpus = calculator.get_recommended_gpus_for_model(
            model_name_or_vram=model_name_or_vram, min_vram_headroom_gb=min_headroom_gb
        )
...
def filter_gpus(min_vram, precision, family, generation, format):
    ...
    if family:
        gpu_configs = [
            gpu for gpu in gpu_configs if gpu["family"].lower() == family.lower()
        ]
    ...
    if generation:
        gpu_configs = [
            gpu
            for gpu in gpu_configs
            if gpu["generation"].lower() == generation.lower()
        ]
