...
from src.advanced_calculator.modules.gpus import get_gpu_config, list_all_gpus
...
def create_app():
    ...
    def standardize_calculator_response(analysis, data):
        ...
        app.logger.info(
            f"[standardize] Received model_info: {json.dumps(model_info, indent=2)}"
        )  # Log model_info
        parameters_billions = model_info.get("parameters_b", 0)
        app.logger.info(
            f"[standardize] Extracted parameters_billions: {parameters_billions}"
        )  # Log extracted value
        ...
        app.logger.info(
            f"Raw VRAM data received for standardization: {json.dumps(raw_vram_data, indent=2)}"
        )

        # Check if the raw VRAM data seems valid (has the structure from calculate_total_vram)
        if (
            isinstance(raw_vram_data, dict)
            and "weights_base" in raw_vram_data
            and "total" in raw_vram_data
        ):
            ...
            std_vram["activations_with_overhead"] = vram.get(
                "activations_with_overhead", 0
            )
            std_vram["total_with_component_overhead"] = vram.get(
                "component_subtotal", 0
            )  # Use the subtotal before system overhead
            std_vram["total_system_wide"] = vram.get(
                "total", 0
            )  # Use the final total including system overhead

            # Extract per_gpu data if available, otherwise calculate for single GPU
            per_gpu_data = vram.get(
                "per_gpu", {}
            )  # Check for potential future 'per_gpu' dict from vram calculator
            num_gpus = analysis.get("parallelism", {}).get(
                "num_gpus", 1
            )  # Use the num_gpus from the standardized parallelism info
            ...
            if isinstance(per_gpu_data, dict) and per_gpu_data and num_gpus > 1:
                ...
                app.logger.warning(
                    "Received per_gpu data structure, but standardization logic needs verification/update based on actual keys."
                )
                std_vram["model_per_gpu"] = per_gpu_data.get("weights_base", 0)
                std_vram["kv_cache_per_gpu"] = per_gpu_data.get("kv_cache_base", 0)
                std_vram["activations_per_gpu"] = per_gpu_data.get(
                    "activations_base", 0
                )
                std_vram["model_per_gpu_with_overhead"] = per_gpu_data.get(
                    "weights_with_overhead", 0
                )
                std_vram["kv_cache_per_gpu_with_overhead"] = per_gpu_data.get(
                    "kv_cache_with_overhead", 0
                )
                std_vram["activations_per_gpu_with_overhead"] = per_gpu_data.get(
                    "activations_with_overhead", 0
                )
                std_vram["total_base_per_gpu"] = per_gpu_data.get("total_base", 0)
                std_vram["total_per_gpu_with_component_overhead"] = per_gpu_data.get(
                    "component_subtotal", 0
                )
                ...
                std_vram["model_per_gpu_with_overhead"] = std_vram[
                    "model_with_overhead"
                ]
                std_vram["kv_cache_per_gpu_with_overhead"] = std_vram[
                    "kv_cache_with_overhead"
                ]
                std_vram["activations_per_gpu_with_overhead"] = std_vram[
                    "activations_with_overhead"
                ]
                std_vram["total_base_per_gpu"] = std_vram["total_base"]
                std_vram["total_per_gpu_with_component_overhead"] = std_vram[
                    "total_with_component_overhead"
                ]
            # --- END MODIFIED PER_GPU LOGIC ---

            app.logger.info(
                f"Successfully standardized VRAM data: {json.dumps(std_vram, indent=2)}"
            )

        else:
            # Fallback: If VRAM data is missing or in an unexpected format, create zeros
            app.logger.warning(
                f"VRAM data missing 'weights_base' or 'total' key or not a dict: {raw_vram_data}"
            )
            std_vram = {
                "model_base": 0,
                "kv_cache_base": 0,
                "activations_base": 0,
                "model_with_overhead": 0,
                "kv_cache_with_overhead": 0,
                "activations_with_overhead": 0,
                "total_base": 0,
                "total_with_component_overhead": 0,
                "total_system_wide": 0,
                "model_per_gpu": 0,
                "kv_cache_per_gpu": 0,
                "activations_per_gpu": 0,
                "model_per_gpu_with_overhead": 0,
                "kv_cache_per_gpu_with_overhead": 0,
                "activations_per_gpu_with_overhead": 0,
                "total_base_per_gpu": 0,
                "total_per_gpu_with_component_overhead": 0,
            }
        ...
        if (
            data.get("parallelism") and std_vram.get("total_system_wide", 0) > 0
        ):  # Only apply if parallelism requested AND we have some VRAM numbers
            ...
            if num_gpus > 1 and std_vram.get(
                "total_per_gpu_with_component_overhead"
            ) == std_vram.get("total_with_component_overhead"):
                app.logger.info(
                    "Applying post-hoc parallelism division to VRAM figures."
                )  # Log if this safety net is triggered
                ...
                if strategy == "tensor":
                    ...
                    std_vram["model_per_gpu_with_overhead"] = (
                        std_vram.get("model_with_overhead", 0) / tp_size
                    )
                elif strategy == "pipeline":
                    std_vram["model_per_gpu"] = std_vram.get("model_base", 0) / pp_size
                    std_vram["model_per_gpu_with_overhead"] = (
                        std_vram.get("model_with_overhead", 0) / pp_size
                    )
                elif strategy == "tensor_pipeline":
                    std_vram["model_per_gpu"] = std_vram.get("model_base", 0) / (
                        tp_size * pp_size
                    )
                    std_vram["model_per_gpu_with_overhead"] = std_vram.get(
                        "model_with_overhead", 0
                    ) / (tp_size * pp_size)
                ...
                std_vram["total_base_per_gpu"] = (
                    ...
                    + std_vram.get(
                        "kv_cache_per_gpu", 0
                    )  # Assumes KV/Activations per_gpu were set correctly earlier or are system wide figures
        ...
        parallelism = {
            ...
            "effective_tflops": float(data.get("gpu", 312.0))
            * int(data.get("parallelism", {}).get("num_gpus", 1)),
        }
        ...
        app.logger.info(
            f"[standardize] Final standardized response: {json.dumps(standardized, indent=2)}"
        )  # Log final standardized dict
    ...
    def unifying_calculator(data):
        ...
        output_seq_length = max(
            1, int(data.get("output_seq_length", 512))
        )  # Ensure minimum of 1
        ...
        try:
            ...
            if (
                gpu_id
                and isinstance(gpu_id, str)
                and not gpu_id.replace(".", "").isdigit()
            ):
                ...
                try:
                    ...
                    app.logger.warning(
                        f"GPU '{gpu_id}' not recognized: {str(e)}. Falling back to TFLOPS."
                    )
                    ...
                    result.append(
                        {
                            ...
                            "tflops": gpu_config.get(
                                "fp16_tflops", gpu_config.get("bf16_tflops", 0)
                            ),
                            "vram": gpu_config.get("vram_gb", 0),
                            "family": gpu_config.get("family", ""),
                            "gen": gpu_config.get("gen", ""),
                            "id": gpu_id,
                            "supported_precisions": gpu_config.get(
    ...
    def model_scaling_analysis():
        ...
        try:
            ...
            hidden_dim = int(
                model_config.get("hidden_dim", data.get("base_hidden_dim", 0))
            )
            ff_dim = int(model_config.get("ff_dim", data.get("base_ff_dim", 0)))
            num_layers = int(
                model_config.get("num_layers", data.get("base_num_layers", 0))
            )
            vocab_size = int(
                model_config.get("vocab_size", data.get("base_vocab_size", 0))
            )
            seq_length = int(
                model_config.get(
                    "seq_len",
                    data.get("sequence_length", data.get("base_seq_length", 2048)),
                )
            )
            ...
            app.logger.info(
                f"Parsed model parameters: hidden_dim={hidden_dim}, ff_dim={ff_dim}, num_layers={num_layers}, vocab_size={vocab_size}, seq_length={seq_length}"
            )
            app.logger.info(
                f"Parsed GPU parameters: vram={gpu_vram_gb}, batch_size={batch_size}, precision={precision}"
            )
            ...
            if "scaling_details" not in scaling_result:
                scaling_result["scaling_details"] = {
                    "note": "No detailed scaling information available"
                }
                ...
                return jsonify(
                    {
                        ...
                        "parameter_count": model_config_data.get(
                            "parameters_billions", 0
                        ),
                    }
                )
    ...
    def calculate():
        ...
        try:
            ...
            if calculation_type == "model":
                ...
                analysis = unifying_calculator(
                    data
                )  # Returns standardized analysis or error dict
                ...
                standardized_response = (
                    analysis  # No need to call standardize_calculator_response again
                )
