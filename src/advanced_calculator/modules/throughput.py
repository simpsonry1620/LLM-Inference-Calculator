...
class ThroughputCalculator:
    ...
    def estimate_inference_throughput(
        ...
        if efficiency_factor > 1.0:
            raise ValueError(
                f"Efficiency factor must be <= 1.0, got {efficiency_factor}"
            )
    ...
    def estimate_token_generation_latency(
        self, flops_per_token: int, gpu_tflops: float, efficiency_factor: float = 0.3
    ) -> float:
        """
    ...
    def estimate_batch_throughput(
        ...
        if num_gpus > 1:
            if interconnect_bandwidth_gb_per_sec is None:
                raise ValueError(
                    "Interconnect bandwidth must be provided for multi-GPU throughput estimation."
                )
            ...
            calculated_parallel_efficiency = max(
                0.4, base_efficiency - efficiency_decay_per_gpu * (num_gpus - 1)
            )
    ...
    def calculate_throughput_across_gpus(
        ...
        for gpu_name in gpu_names:
            ...
            tflops = gpu_config.get(
                "fp16_tflops",
                gpu_config.get("bf16_tflops", gpu_config.get("fp32_tflops", 0)),
            )
