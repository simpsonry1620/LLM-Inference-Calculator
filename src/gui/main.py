...
def update_tflops_entry(event, widgets):
    ...
    if tflops_value is not None:
        ...
        tflops_entry.insert(
            0, "N/A"
        )  # Indicate TFLOPS not available for this precision
...
def calculate_requirements(widgets):
    ...
    for key in [
        ...
        "weights_vram_value",
        "kv_cache_vram_value",
        "activations_vram_value",
        "system_overhead_value",
        "flops_per_token_value",
        "prefill_latency_value",
        "token_latency_value",
        "ttft_value",
        "trt_value",
        "tokens_per_sec_value",
        "parallelism_strategy_value",
        "parallelism_num_gpus_value",
        "parallelism_tp_degree_value",
        "parallelism_pp_degree_value",
    ...
    try:
        if use_predefined_model:
            ...
            widgets["hidden_dim_entry"].insert(
                0, str(model_params.get("hidden_dimensions", ""))
            )
            widgets["ff_dim_entry"].delete(0, tk.END)
            widgets["ff_dim_entry"].insert(
                0, str(model_params.get("feedforward_dimensions", ""))
            )
            widgets["num_layers_entry"].delete(0, tk.END)
            widgets["num_layers_entry"].insert(
                0, str(model_params.get("num_layers", ""))
            )
            widgets["vocab_size_entry"].delete(0, tk.END)
            widgets["vocab_size_entry"].insert(
                0, str(model_params.get("vocab_size", ""))
            )
            ...
            model_params["hidden_dimensions"] = get_int_from_entry(
                widgets["hidden_dim_entry"]
            )
            model_params["feedforward_dimensions"] = get_int_from_entry(
                widgets["ff_dim_entry"]
            )
        ...
        input_sequence_length = get_int_from_entry(
            widgets["input_seq_len_entry"], default_value=2048
        )
        output_sequence_length = get_int_from_entry(
            widgets["output_seq_len_entry"], default_value=128
        )
        ...
        if gpu_tflops_str.lower() != "n/a":
            gpu_tflops = get_float_from_entry(
                widgets["gpu_tflops_entry"], default_value=0.0
            )
        else:
            ...
            if selected_gpu != "Custom":
                ...
                if gpu_config:
                    ...
                    if prec_lower == "fp16":
                        tflops_key = "fp16_tflops"
                    elif prec_lower == "bf16":
                        tflops_key = (
                            "bf16_tflops"
                            if "bf16_tflops" in gpu_config
                            else "fp16_tflops"
                        )
                    elif prec_lower == "fp32":
                        tflops_key = "fp32_tflops"
                    elif prec_lower == "int8":
                        tflops_key = "int8_tflops"
                    elif prec_lower == "int4":
                        tflops_key = "int4_tflops"
                    ...
                    if tflops_key and tflops_key in gpu_config:
                        ...
                        widgets["gpu_tflops_entry"].config(
                            state=tk.DISABLED if selected_gpu != "Custom" else tk.NORMAL
                        )  # Re-disable if needed
                        status_label.config(text="Status: Found TFLOPS, Calculating...")
                        root.update_idletasks()
                    else:
                        raise ValueError(
                            f"TFLOPS for {selected_gpu} @ {precision} not found in config."
                        )
                else:
                    raise ValueError(f"GPU config for {selected_gpu} not found.")
            else:  # Custom GPU and TFLOPS is N/A
                raise ValueError(
                    "GPU TFLOPS not available or specified for Custom GPU."
                )

        efficiency_pct = get_float_from_entry(
            widgets["efficiency_entry"], default_value=30.0
        )
        efficiency_factor = (
            efficiency_pct / 100.0 if efficiency_pct is not None else None
        )

        # Check if any input conversion failed
        input_values = [
            model_params.get("hidden_dimensions"),
            model_params.get("feedforward_dimensions"),
            model_params.get("num_layers"),
            model_params.get("vocab_size"),
            batch_size,
            input_sequence_length,
            output_sequence_length,
            gpu_tflops,
            efficiency_factor,  # Add new performance inputs
        ]
        ...
        # Fetch Model Config early to get parameters for latency calculation
        model_config = None
        model_parameters_billion = None  # Default for custom models
        if use_predefined_model:
            model_config = calculator.get_model_config(selected_model)
            if model_config:
                model_parameters_billion = model_config.get("parameter_count")
                if model_parameters_billion is None:
                    # This case shouldn't happen if config is valid, but handle it
                    messagebox.showwarning(
                        "Model Info Missing",
                        f"Parameter count missing for model '{selected_model}'. Latency calculation might be less accurate.",
                    )
                    model_parameters_billion = (
                        0.0  # Or None, depending on how latency calc handles it
                    )
            else:
                # This was already checked, but redundant check is safe
                raise ValueError(
                    f"Could not retrieve config for selected model '{selected_model}' again."
                )

        # Initialize bandwidth before the conditional block
        gpu_bandwidth_gb_per_sec = 0.0

        # Original block for fetching GPU config, TFLOPS update, and bandwidth
        selected_gpu_name = widgets["gpu_var"].get()
        gpu_config = None
        if selected_gpu_name != "Custom":
            gpu_config = calculator.get_gpu_config(selected_gpu_name)
            if gpu_config:
                # --- Update TFLOPS based on precision if not Custom GPU ---
                # (Existing logic for updating TFLOPS based on config remains here)
                tflops_key = None
                prec_lower = precision.lower()
                if prec_lower == "fp16":
                    tflops_key = "fp16_tflops"
                    ...
                    tflops_key = (
                        "bf16_tflops" if "bf16_tflops" in gpu_config else "fp16_tflops"
                    )
                elif prec_lower == "fp32":
                    tflops_key = "fp32_tflops"
                elif prec_lower == "int8":
                ...
                if tflops_key and tflops_key in gpu_config:
                    gpu_tflops = gpu_config[tflops_key]
                    # Update entry visually only if it changed from 'N/A' logic earlier
                    if widgets["gpu_tflops_entry"].get().lower() == "n/a":
                        widgets["gpu_tflops_entry"].config(state=tk.NORMAL)
                        widgets["gpu_tflops_entry"].delete(0, tk.END)
                        widgets["gpu_tflops_entry"].insert(0, str(gpu_tflops))
                        widgets["gpu_tflops_entry"].config(state=tk.DISABLED)
                # --------------------------------------------------------------

                # --- Get bandwidth if config exists ---
                gpu_bandwidth_gb_per_sec = gpu_config.get(
                    "memory_bandwidth_gb_per_sec", 0.0
                )
                if gpu_bandwidth_gb_per_sec == 0.0:
                    messagebox.showwarning(
                        "GPU Info Missing",
                        f"Memory bandwidth info missing for {selected_gpu_name}. Latency calculations might be inaccurate.",
                    )
                # --------------------------------------
                ...
                messagebox.showwarning(
                    "GPU Info Missing",
                    f"Configuration for selected GPU '{selected_gpu_name}' not found. Cannot determine bandwidth.",
                )
                # Allow editing TFLOPS if GPU info is bad
                widgets["gpu_tflops_entry"].config(state=tk.NORMAL)
        else:
            # Custom GPU - ensure TFLOPS entry is enabled
            widgets["gpu_tflops_entry"].config(state=tk.NORMAL)
            # Bandwidth remains 0.0 for custom

        # Ensure FF dim is present (Estimate if missing)
        if not model_params.get("feedforward_dimensions") and model_params.get(
            "hidden_dimensions"
        ):
            model_params["feedforward_dimensions"] = (
                model_params["hidden_dimensions"] * 4
            )
        ...
        prefill_latency_s = calculator.calculate_prefill_latency(
            flops_prefill, gpu_tflops, efficiency_factor
        )
        token_latency_s = calculator.calculate_token_latency(
            flops_per_token=flops_per_token,
            gpu_tflops=gpu_tflops,
            efficiency_factor=efficiency_factor,
            gpu_bandwidth_gb_per_sec=gpu_bandwidth_gb_per_sec,  # Use fetched variable
            precision=precision,
            model_parameters_billion=model_parameters_billion,  # Pass model params
        )
        ...
        widgets["weights_vram_value"].config(
            text=f"{vram_results.get('weights_with_overhead', 0.0):.2f} GB"
        )
        widgets["kv_cache_vram_value"].config(
            text=f"{vram_results.get('kv_cache_with_overhead', 0.0):.2f} GB"
        )
        widgets["activations_vram_value"].config(
            text=f"{vram_results.get('activations_with_overhead', 0.0):.2f} GB"
        )
        widgets["system_overhead_value"].config(
            text=f"{vram_results.get('system_overhead_applied', 0.0):.2f} GB"
        )

        # FLOPs Results
        widgets["flops_per_token_value"].config(
            text=f"{flops_per_token / 1e9:.2f} GFLOPs"
        )

        # Latency/Throughput Results (Convert s to ms where appropriate)
        widgets["prefill_latency_value"].config(
            text=f"{prefill_latency_s * 1000:.2f} ms"
        )
        ...
        if selected_gpu_name != "Custom":
            ...
            if gpu_config and "vram_gb" in gpu_config:
                ...
                print(
                    f"[DEBUG] Required VRAM: {total_vram_required_gb:.2f} GB"
                )  # DEBUG

                if total_vram_required_gb > gpu_vram_gb:
                    print(
                        "[DEBUG] Required VRAM > GPU VRAM. Attempting scaling calculation..."
                    )  # DEBUG
                    status_label.config(text="Status: Determining scaling...")
                    root.update_idletasks()
                    try:
                        # Fetch interconnect bandwidth for scaling calculation
                        interconnect_bw = gpu_config.get(
                            "interconnect_bandwidth_gb_per_sec"
                        )  # Changed key? Check definition
                        if interconnect_bw is None or interconnect_bw == 0.0:
                            # Option 2: Error out / Indicate unable to calculate
                            messagebox.showerror(
                                "Scaling Error",
                                f"Interconnect bandwidth info missing for {selected_gpu_name}. Cannot determine scaling accurately.",
                            )
                            raise ValueError(
                                f"Interconnect bandwidth info missing for {selected_gpu_name}"
                            )
                        ...
                        scaling_info = calculator.determine_model_scaling(
                            ...
                            hidden_dimensions=model_params["hidden_dimensions"],
                            feedforward_dimensions=model_params[
                        ...
                        messagebox.showerror(
                            "Scaling Error", f"Could not determine scaling: {scale_e}"
                        )
                ...
                messagebox.showwarning(
                    "GPU Info Missing",
                    f"VRAM info for selected GPU '{selected_gpu_name}' not found. Cannot determine scaling.",
                )
        else:
        ...
        if scaling_info:
            ...
            # Fits on single GPU
            widgets["parallelism_text"].insert(
                tk.END, "Model fits on single selected GPU."
            )
        elif selected_gpu_name == "Custom":
            widgets["parallelism_text"].insert(
                tk.END, "N/A (Select a specific GPU to determine scaling)"
            )
        else:  # Predefined GPU but VRAM info missing
            widgets["parallelism_text"].insert(
                tk.END,
                "Could not determine scaling (Missing VRAM info for selected GPU).",
            )
        widgets["parallelism_text"].config(state=tk.DISABLED)
...
def on_model_select(event, widgets):
    ...
    if not is_custom:
        ...
        if config:
            ...
            widgets["hidden_dim_entry"].delete(0, tk.END)
            widgets["hidden_dim_entry"].insert(
                0, str(config.get("hidden_dimensions", ""))
            )
            widgets["ff_dim_entry"].delete(0, tk.END)
            widgets["ff_dim_entry"].insert(
                0, str(config.get("feedforward_dimensions", ""))
            )
            widgets["num_layers_entry"].delete(0, tk.END)
...
def main():
    ...
    row_index = 0
    # Model Selection
    ttk.Label(input_frame, text="Model:").grid(
        column=0, row=row_index, sticky=tk.W, padx=5, pady=5
    )
    ...
    model_combobox.bind(
        "<<ComboboxSelected>>", lambda event: on_model_select(event, widgets)
    )
    widgets["model_combobox"] = model_combobox
    row_index += 1

    # Hidden Dimensions
    ttk.Label(input_frame, text="Hidden Dimensions:").grid(
        column=0, row=row_index, sticky=tk.W, padx=5, pady=5
    )
    ...
    ttk.Label(input_frame, text="Feedforward Dim:").grid(
        column=0, row=row_index, sticky=tk.W, padx=5, pady=5
    )
    ...
    # Number of Layers
    ttk.Label(input_frame, text="Number of Layers:").grid(
        column=0, row=row_index, sticky=tk.W, padx=5, pady=5
    )
    ...
    # Vocabulary Size
    ttk.Label(input_frame, text="Vocabulary Size:").grid(
        column=0, row=row_index, sticky=tk.W, padx=5, pady=5
    )
    ...
    # Precision
    ttk.Label(input_frame, text="Precision:").grid(
        column=0, row=row_index, sticky=tk.W, padx=5, pady=5
    )
    ...
    precision_combobox.grid(
        column=1, row=row_index, sticky=(tk.W, tk.E), padx=5, pady=5
    )
    precision_combobox.set("fp16")
    precision_combobox.bind(
        "<<ComboboxSelected>>", lambda event: update_tflops_entry(event, widgets)
    )  # Bind handler
    widgets["precision_combobox"] = precision_combobox
    row_index += 1

    # GPU Selection (New)
    ttk.Label(input_frame, text="GPU:").grid(
        column=0, row=row_index, sticky=tk.W, padx=5, pady=5
    )
    gpu_var = tk.StringVar()
    widgets["gpu_var"] = gpu_var
    gpu_names = ["Custom"] + calculator.get_available_gpus()
    gpu_combobox = ttk.Combobox(
        input_frame, textvariable=gpu_var, values=gpu_names, width=25, state="readonly"
    )
    ...
    ttk.Label(input_frame, text="GPU TFLOPS (@Precision):").grid(
        column=0, row=row_index, sticky=tk.W, padx=5, pady=5
    )
    ...
    # Compute Efficiency
    ttk.Label(input_frame, text="Compute Efficiency (%):").grid(
        column=0, row=row_index, sticky=tk.W, padx=5, pady=5
    )
    ...
    calculate_button = ttk.Button(
        main_frame, text="Calculate", command=lambda: calculate_requirements(widgets)
    )
    ...
    ttk.Label(results_frame, text="VRAM:", font=("Segoe UI", 9, "bold")).grid(
        column=0, row=res_row, sticky=tk.W, padx=5, pady=(5, 2)
    )
    res_row += 1

    ttk.Label(results_frame, text="  Total Required:").grid(
        column=0, row=res_row, sticky=tk.W, padx=15, pady=1
    )  # Indented
    ...
    ttk.Label(results_frame, text="  Weights VRAM:").grid(
        column=0, row=res_row, sticky=tk.W, padx=15, pady=1
    )
    ...
    ttk.Label(results_frame, text="  KV Cache VRAM:").grid(
        column=0, row=res_row, sticky=tk.W, padx=15, pady=1
    )
    ...
    ttk.Label(results_frame, text="  Activations VRAM:").grid(
        column=0, row=res_row, sticky=tk.W, padx=15, pady=1
    )
    activations_vram_value = ttk.Label(results_frame, text="-", anchor=tk.W)
    activations_vram_value.grid(
        column=1, row=res_row, sticky=(tk.W, tk.E), padx=5, pady=1
    )
    widgets["activations_vram_value"] = activations_vram_value
    res_row += 1

    ttk.Label(results_frame, text="  System Overhead:").grid(
        column=0, row=res_row, sticky=tk.W, padx=15, pady=1
    )
    system_overhead_value = ttk.Label(results_frame, text="-", anchor=tk.W)
    system_overhead_value.grid(
        column=1, row=res_row, sticky=(tk.W, tk.E), padx=5, pady=1
    )
    ...
    # FLOPs Section
    ttk.Label(results_frame, text="FLOPs:", font=("Segoe UI", 9, "bold")).grid(
        column=0, row=res_row, sticky=tk.W, padx=5, pady=(5, 2)
    )
    res_row += 1

    ttk.Label(results_frame, text="  Per Output Token:").grid(
        column=0, row=res_row, sticky=tk.W, padx=15, pady=1
    )
    flops_per_token_value = ttk.Label(results_frame, text="-", anchor=tk.W)
    flops_per_token_value.grid(
        column=1, row=res_row, sticky=(tk.W, tk.E), padx=5, pady=1
    )
    ...
    # Performance Section
    ttk.Label(results_frame, text="Performance:", font=("Segoe UI", 9, "bold")).grid(
        column=0, row=res_row, sticky=tk.W, padx=5, pady=(5, 2)
    )
    res_row += 1

    ttk.Label(results_frame, text="  Prefill Latency:").grid(
        column=0, row=res_row, sticky=tk.W, padx=15, pady=1
    )
    prefill_latency_value = ttk.Label(results_frame, text="-", anchor=tk.W)
    prefill_latency_value.grid(
        column=1, row=res_row, sticky=(tk.W, tk.E), padx=5, pady=1
    )
    widgets["prefill_latency_value"] = prefill_latency_value
    res_row += 1

    ttk.Label(results_frame, text="  Token Latency:").grid(
        column=0, row=res_row, sticky=tk.W, padx=15, pady=1
    )
    ...
    ttk.Label(results_frame, text="  Time to First Token:").grid(
        column=0, row=res_row, sticky=tk.W, padx=15, pady=1
    )
    ...
    ttk.Label(results_frame, text="  Total Request Time:").grid(
        column=0, row=res_row, sticky=tk.W, padx=15, pady=1
    )
    ...
    ttk.Label(results_frame, text="  Tokens per Second:").grid(
        column=0, row=res_row, sticky=tk.W, padx=15, pady=1
    )
    tokens_per_sec_value = ttk.Label(results_frame, text="-", anchor=tk.W)
    tokens_per_sec_value.grid(
        column=1, row=res_row, sticky=(tk.W, tk.E), padx=5, pady=1
    )
    ...
    ttk.Label(
        results_frame, text="Parallelism Info:", font=("Segoe UI", 9, "bold")
    ).grid(column=0, row=res_row, sticky=tk.W, padx=5, pady=(5, 2))
    res_row += 1

    parallelism_text = scrolledtext.ScrolledText(
        results_frame, wrap=tk.WORD, height=8, width=40, state=tk.DISABLED
    )
    parallelism_text.grid(
        column=0, row=res_row, columnspan=2, sticky=(tk.W, tk.E), padx=5, pady=5
    )
    ...
    status_label = ttk.Label(
        main_frame, text="Status: Ready", relief=tk.SUNKEN, anchor=tk.W
    )
    ...
    update_tflops_entry(
        None, widgets
    )  # Initial TFLOPS fill based on default Custom/fp16
